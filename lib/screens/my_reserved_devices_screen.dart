import 'package:booking_app_client/constants.dart';
import 'package:booking_app_client/providers/main_provider.dart';
import 'package:booking_app_client/widgets_model/custom_alert_dialog.dart';
import 'package:booking_app_client/widgets_model/custom_list_tile_profile.dart';
import 'package:booking_app_client/widgets_model/custom_text.dart';
import 'package:booking_app_client/widgets_model/main_drawer.dart';
import 'package:double_back_to_close/double_back_to_close.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class MyReservedDevicesScreen extends StatefulWidget {
  @override
  _MyReservedDevicesScreenState createState() =>
      _MyReservedDevicesScreenState();
}

class _MyReservedDevicesScreenState extends State<MyReservedDevicesScreen> {
  bool isLoading = false;

  @override
  Widget build(BuildContext context) {
    return DoubleBack(
      child: Scaffold(
        appBar: AppBar(
          title: Text('My Reserved Devices'),
          centerTitle: true,
        ),
        body: isLoading
            ? Center(
                child: CircularProgressIndicator(),
              )
            : Consumer<MainProvider>(
                builder: (context, value, child) => value
                        .reservedDevicesList.isEmpty
                    ? Center(
                        child: CustomText(
                          text: 'No device booked yet',
                          fontSize: 22,
                          alignment: Alignment.center,
                        ),
                      )
                    : ListView.separated(
                        itemBuilder: (context, index) => CustomListTileProfile(
                              title:
                                  value.reservedDevicesList[index].deviceName,
                              titleColor: Colors.black,
                              leading: CircleAvatar(
                                child: FittedBox(
                                    child: Padding(
                                  padding: const EdgeInsets.all(8.0),
                                  child: Text(
                                      value.reservedDevicesList[index].type),
                                )),
                              ),
                              subtitle: 'From: ' +
                                  value.reservedDevicesList[index].startDate +
                                  '\nTo: ' +
                                  value.reservedDevicesList[index].endDate,
                              subtitleColor: Colors.grey,
                              trailing: IconButton(
                                icon: Icon(Icons.remove_circle),
                                color: KPrimaryColor,
                                onPressed: () async {
                                  showDialog(
                                      context: context,
                                      builder: (context) => CustomAlertDialog(
                                            title: 'Are you sure!',
                                            content: 'Remove the reservation ?',
                                            onPressedYes: () {
                                              setState(() {
                                                isLoading = true;
                                              });

                                              Provider.of<MainProvider>(context,
                                                      listen: false)
                                                  .unBookedDevice(
                                                      value.reservedDevicesList[
                                                          index])
                                                  .then((value) {
                                                ScaffoldMessenger.of(context)
                                                    .showSnackBar(SnackBar(
                                                        content: Text(
                                                            'Reservation has been deleted')));
                                                setState(() {
                                                  isLoading = false;
                                                });
                                                Navigator.of(context).pop();
                                              });
                                            },
                                            onPressedNo: () =>
                                                Navigator.of(context).pop(),
                                          ));
                                },
                              ),
                            ),
                        separatorBuilder: (context, index) => Divider(),
                        itemCount: value.reservedDevicesList.length),
              ),
        drawer: MainDrawer(),
      ),
    );
  }
}
